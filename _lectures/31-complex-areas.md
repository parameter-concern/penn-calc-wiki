...
(:latex:) \begin{align*} y-x &= 0
y^2+x &= 2. \end{align*} (:latexend:)
...
(:latex:) \[ y = 2-y^2. \] (:latexend:)

A rearranging and factoring gives

(:latex:) \[ y^2+y-2 = (y-1)(y+2) = 0, \] (:latexend:)
...
(:latex:) \[ dA = \begin{cases} (x + \sqrt{2-x}) \, dx & \hbox{if $-2 \leq x \leq 1$}
2 \sqrt{2-x} \, dx & \hbox{if $1 \leq x \leq 2$} \end{cases} \] (:latexend:)
...
&= \frac{3}{2}y^2 - \frac{1}{3}y^3+4y \bigg|^4_{-1}
&= \frac{125}{6} \end{align*} (:latexend:)

Example

Find the area of the region bounded by ($y = \ln(x)$) and the lines ($y = 0$), ($y=1$), and ($x=0$). (:toggle hide show="Answer" box2:)
...
(:latex:) \[ dA = \frac{1}{2}(r)(r\, d \theta) = \frac{1}{2} (f(\theta))^2 \, d\theta, \] (:latexend:)

since ($r = f(\theta)$). Thus, the area of a polar region defined by ($r = f(\theta)$), where ($a \leq \theta \leq b$), is

(:latex:) \[ A = \int_{\theta=a}^b \frac{1}{2} (f(\theta))^2 \, d\theta. \] (:latexend:)

Example

Compute the area of the cardioid ($r = 1+\cos \theta$). (:toggle hide show="Answer" box4:)

In this case, ($f(\theta) = 1 + \cos \theta$), and so the area element is

(:latex:) \begin{align*} dA &= \frac{1}{2}(1+ \cos \theta)^2 \, d\theta
&= \frac{1}{2} (1 + 2 \cos \theta + \cos^2 \theta) \, d\theta \end{align*} (:latexend:)
...
&= \frac{1}{2} \left( \theta + 2 \sin \theta + \frac{1}{2} \theta + \frac{1}{4} \sin(2 \theta) \right) \bigg|_{\theta=0}^{2\pi}
&= \frac{3\pi}{2}. \end{align*} (:latexend:)

Example

Find the area of a single petal of the polar curve ($r = \sin(3\theta)$):

Hint: To find the bounds on ($\theta$), compute when ($r=0$). (:toggle hide show="Answer" box5:)
...
&= \frac{1}{4} \left( \theta - \frac{1}{6} \sin(6 \theta) \right) \bigg|_{\theta=0}^{\pi/3}
&= \frac{\pi}{12} \end{align*} (:latexend:)

Example

Find the area inside the circle ($r = 2 \sin \theta$) and outside the circle ($r = 1$):

(:toggle hide show="Answer" box5a:)

First, we find the intersections by setting the curves equal, which gives

(:latex:) \[ 2\sin \theta = 1 \quad \Rightarrow \quad \sin \theta = \frac{1}{2}, \] (:latexend:)
...
(:latex:) \[ dA = \left(\frac{1}{2}(2 \sin \theta)^2 - \frac{1}{2}(1)^2 \right) \, d \theta. \] (:latexend:)
...
&= \frac{1}{2} \left(\theta - \sin(2 \theta) \right) \bigg|_{\pi/6}^{5\pi/6}
&= \frac{\pi}{3} + \frac{\sqrt{3}}{2}. \end{align*} (:latexend:)

From the second to the third line above, we used the power reduction formula for sine:

(:latex:) \[ \sin^2 \theta = \frac{1}{2}(1-\cos(2\theta)). \] (:latexend:)
EXERCISES
