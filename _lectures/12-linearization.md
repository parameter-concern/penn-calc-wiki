...
(:latex:) \[ A(x+h) = (x+h)^2 = x^2+2xh+h^2. \] (:latexend:)
...
(:latex:) \[ A(x+h) = \frac{1}{2}x^2 + xh + \frac{1}{2}h^2. \] (:latexend:)
...
(:latex:) \[ A(x+h) = \pi (x+h)^2 = \pi x^2 + 2\pi x h + \pi h^2. \] (:latexend:)
...
(:latex:) \[ f(a+h) \approx f(a) + f'(a)h. \] (:latexend:)
...
Using a linear approximation, estimate ($\sqrt{250}$). (:toggle hide show="Answer" box0:)
...
(:latex:) \begin{align*} f(x) &\approx f(a) + f'(a)h
\sqrt{ 250 } &\approx \sqrt{256} + \frac{1}{2\sqrt{256}}(-6)
&= 16 - \frac{6}{32}
&= 15 \frac{13}{16}
& \approx 15.8, \end{align*} (:latexend:)

which is very close to the calculator's answer of ($15.811 \ldots $).

Example

Using a linear approximation, estimate ($\sqrt{104}$). Is this an over-approximation or an under-approximation? (:toggle hide show="Answer" box1:)
...
(:latex:) \begin{align*} f(a + h) &\approx f(a) + f'(a)h
&= \sqrt{100} + \frac{1}{2\sqrt{100}} \cdot 4
&= 10 + \frac{4}{20}
&= 10.2. \end{align*} (:latexend:)
...
Approximate ($\pi^{20}$). Hint: ($\pi^2 \approx 9.86$). (:toggle hide show="Answer" box1a:)
...
(:latex:) \begin{align*} f(9.86) &\approx f(10) + f'(10)(-.14)
&= 10^{10} + 10 (10)^9 (-.14)
&= 10^{10} \left(1-.14 \right)
&= 8.6 \cdot 10^9. \end{align*} (:latexend:)

The true answer is approximately ($8.77 \cdot 10^9$), so this estimate is within 2%.

Example

Approximate ($e^{30}$). Hint: ($e^{3} \approx 20.1$), and ($2^{10} \approx 1000$). (:toggle hide show="Answer" box2:)

From the first hint, ($e^{30} = (e^3)^{10} \approx (20.1)^{10}$), so consider the linear approximation for ($f(x) = x^{10}$) near ($a=20$):

(:latex:) \begin{align*} f(x) & \approx f(20) + f'(20)(x-20)
x^{10} &\approx 20^{10} + 10 \cdot 20^9 (x-20) \end{align*} (:latexend:)

So it follows that

(:latex:) \begin{align*} e^{30} & \approx (20.1)^{10}
& \approx 20^{10} + 10 \cdot 20^9 \cdot (.1)
& \approx 20^{10}\left[1+(.5)(.1)\right]
& \approx 2^{10} \cdot 10^{10} \cdot 1.05
& \approx 1.05 \cdot 10^{13} \end{align*} (:latexend:)
...
(:latex:) \begin{align*} f(x+h) &= f(x) + f'(x)h + O(h^2)
&\approx f(x) + f'(x)h. \end{align*} (:latexend:)
...
(:latex:) \[ f(x) + f'(x)h = 0 \quad \Rightarrow \quad h = -\frac{f(x)}{f'(x)}. \] (:latexend:)

Thus, an even closer guess of a root is

(:latex:) \[ x + h = x - \frac{f(x)}{f'(x)}. \] (:latexend:)
...
(:latex:) \begin{equation*} x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)} \end{equation*} (:latexend:)
...
Find the update rule for approximating ($\frac{1}{a}$), the reciprocal of a number ($a$). (:toggle hide show="Answer" box2b:)
...
(:latex:) \[ x_{n+1} = \frac{1}{a}, \] (:latexend:)
...
(:latex:) \begin{align*} x_{n+1} &= x_n - \frac{f(x_n)}{f'(x_n)}
&= x_n - \frac{a-\frac{1}{x_n}}{\frac{1}{x_n^2}}
&= x_n - \left(x_n^2 a - x_n\right)
&= 2x_n - a x_n^2. \end{align*} (:latexend:)

Note that this rule does not involve division, but only multiplication and subtraction.

Example

Find the update rule for using Newton's method to approximate ($\sqrt{a}$). Use the update rule twice with initial guess ($x_0 = 3$) to estimate ($\sqrt{11}$). (:toggle hide show="Answer" box3:)
...
&= \frac{x_n}{2} + \frac{a}{2x_n}. \end{align*} (:latexend:)

Using the update rule with ($x_0 = 3$) and ($a = 11$) gives

(:latex:) \begin{align*} x_1 &= \frac{x_0}{2} + \frac{11}{2x_0}
&= \frac{3}{2} + \frac{11}{6}
&= \frac{20}{6}
&= \frac{10}{3} \end{align*} (:latexend:)

Updating one more time gives

(:latex:) \begin{align*} x_2 &= \frac{x_1}{2} + \frac{11}{2x_1}
&= \frac{5}{3} + \frac{11}{\frac{20}{3}}
&= \frac{199}{60}
& \approx 3.3166 \end{align*} (:latexend:)
...
Find the update rule for finding ($\sqrt[3]{a}$). Use the update rule once with the initial guess of ($x_0 = 5$) to estimate ($\sqrt[3]{100}$). (:toggle hide show="Answer" box2c:)
...
&= \frac{2}{3}x_n + \frac{a}{3x_n^2}. \end{align*} (:latexend:)

Using this to estimate ($\sqrt[3]{100}$) with the initial guess ($x_0 = 5$), one finds

(:latex:) \begin{align*} x_1 &= \frac{2}{3}\cdot x_0 + \frac{100}{3 x_0^2}
&= \frac{10}{3} + \frac{4}{3}
&= \frac{14}{3}
& \approx 4.666 \end{align*} (:latexend:)
