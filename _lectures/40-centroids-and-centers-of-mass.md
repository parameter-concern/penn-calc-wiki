...
(:latex:) \begin{equation*} \hbox{Area } = \iint_R dx\,dy. \end{equation*} (:latexend:)
...
Express the area of the region bounded by the curves ($y=x^2-4x+5$) and ($y = x+1$) as a double integral and evaluate the integral. (:toggle hide show="Answer" box1:)
...
(:latex:) \begin{align*} \int_{x=1}^{x=4} \int_{y=x^2-4x+5}^{y=x+1} \,dy\,dx &= \int_{x=1}^{x=4} \left(y \bigg|^{x+1}_{x^2-4x+5} \right) \,dx
&= \int_{x=1}^{x=4} (x+1-(x^2-4x+5)) \,dx
&= \int_{x=1}^{x=4} (-x^2+5x-4)\,dx
&= -\frac{x^3}{3} + \frac{5}{2}x^2 - 4x \bigg|^4_1
&= \frac{9}{2}. \end{align*} (:latexend:)
Centroid
...
(:latex:) \begin{align*} \overline x &= \displaystyle \frac{ \iint_R x \, dx\,dy}{\iint_R \, dx\,dy}
\overline y &= \displaystyle \frac{ \iint_R y \, dx\,dy}{\iint_R \, dx\,dy}. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \overline x &= \frac{ \int_a^b \int_{g(x)}^{f(x)} x dydx}{\int_a^b \int_{g(x)}^{f(x)} dydx}
&= \frac{ \int_a^b x(f(x)-g(x))dx}{\int_a^b (f(x)-g(x))dx}. \end{align*} (:latexend:)

Similarly,

(:latex:) \begin{align*} \overline y &= \frac{\int_a^b \int_{g(x)}^{f(x)} y \,dy\,dx}{\int_a^b \int_{g(x)}^{f(x)} \,dy\,dx}
&= \frac{ \int_a^b \frac{1}{2}(f(x)^2-g(x)^2)\,dx}{\int_a^b(f(x)-g(x))\,dx}. \end{align*} (:latexend:)

Note that the denominator in each case is the area of the region.

Example

Find the centroid of a triangle with vertices at ($(a,0), (b,0),$) and ($(0,c)$). (:toggle hide show="Answer" box2:)
...
(:latex:) \begin{align*} \overline x &= \frac{\int_{y=0}^{y=c} \int_{x=\frac{-b}{c}y+b}^{x = \frac{-a}{c}y+a} x \,dx\,dy}{\int_{y=0}^{y=c} \int_{x=\frac{-b}{c}y+b}^{x = \frac{-a}{c}y+a}\,dx\,dy}
&= \frac{\int_{y=0}^{y=c} \int_{x=\frac{-b}{c}y+b}^{x = \frac{-a}{c}y+a} x\,dx\,dy}{\hbox{Area}} \end{align*} (:latexend:)

Noting that the area of the triangle is ($\frac{1}{2}(a-b)c$), one finds

(:latex:) \begin{align*} \overline x &= \frac{2}{(a-b)c} \int_{y=0}^{y=c} \int_{x=\frac{-b}{c}y+b}^{x = \frac{-a}{c}y+a} x\,dx\,dy
&= \frac{2}{(a-b)c} \int_{y=0}^{y=c} \frac{1}{2}\left((\frac{-a}{c}y+a)^2-(\frac{-b}{c}y+b)^2\right)\,dy
&= \frac{1}{(a-b)c} \cdot \frac{1}{3}\left((\frac{-a}{c}y+a)^3 \frac{-c}{a}-(\frac{-b}{c}y+b)^3 \frac{-c}{b}\right) \bigg|^c_0
&= \frac{1}{3(a-b)c} (a^2 c - b^2 c)
&= \frac{1}{3(a-b)c} c(a+b)(a-b)
&= \frac{1}{3}(a+b). \end{align*} (:latexend:)
...
(:latex:) \[ (\overline x, \overline y) = \left(\frac{x_0+x_1+x_2}{3}, \frac{y_0+y_1+y_2}{3}\right). \] (:latexend:)
...
(:latex:) \begin{align*} \overline{y} &= \frac{2}{\pi R^2} \int_{x=-R}^R \frac{1}{2}(\sqrt{R^2-x^2})^2 \, dx
&= \frac{1}{\pi R^2} \left( R^2 x - \frac{1}{3}x^3\right) \bigg|_{x=-R}^R
&= \frac{1}{\pi R^2} \cdot \frac{4 R^3}{3}
&= \frac{4R}{3\pi}. \end{align*} (:latexend:)

Example

Compute the centroid of the quarter circle of radius ($R$):

(:toggle hide show="Answer" box4a:)

We know that the area of the region is ($\frac{1}{4}\pi R^2$). So we have that

(:latex:) \begin{align*} \overline{x} &= \frac{1}{A} \int x(f(x)-g(x)) \, dx
&= \frac{4}{\pi R^2} \int_{x=0}^R x(\sqrt{R^2-x^2} - 0) \, dx. \end{align*} (:latexend:)

Making a substitution of

(:latex:) \begin{align*} u &= R^2 - x^2
du &= -2x \, dx \end{align*} (:latexend:)

gives

(:latex:) \begin{align*} \frac{4}{\pi R^2} \int_{x=0}^R x \sqrt{R^2-x^2} \, dx &= \frac{4}{\pi R^2} \int_{u=R^2}^0 -\frac{1}{2} \sqrt{u} \, du
&= \frac{-2}{\pi R^2} \frac{2}{3} u^{3/2} \bigg|_{u=R^2}^0
&= \frac{2}{\pi R^2} \cdot \frac{2}{3} R^3
&= \frac{4 R}{3 \pi}. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \overline{y} &= \frac{1}{A} \int \frac{1}{2}(f(x)^2 - g(x)^2) \, dx
&= \frac{2}{\pi R^2} \int_{x=0}^R (R^2 - x^2) \, dx
&= \frac{2}{\pi R^2} \left(R^2 x - \frac{1}{3}x^3\right) \bigg|_{x=0}^R
...
(:latex:) \begin{align*} \overline x &= \frac{\iint_R \rho(x,y)x\,dx\,dy}{\iint_R \rho(x,y) \,dx\,dy}
\overline y &= \frac{\iint_R \rho(x,y)y\,dx\,dy}{\iint_R \rho(x,y) \,dx\,dy}. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} M &= \iint_R \rho(x,y) \,dy\,dx
&= \int_{x=0}^4 \int_{y=0}^{4x-x^2} 2x \, dy \, dx
&= \int_{x=0}^4 \left(2xy \bigg|_{y=0}^{4x-x^2} \right) \, dx
&= \int_{x=0}^4 2x(4x-x^2) \, dx
&= \int_{x=0}^4 (8x^2 - 2x^3) \, dx
&= \frac{8}{3}x^3 - \frac{1}{2}x^4 \bigg|_{x=0}^4
&= \frac{512}{3} - 128 = \frac{128}{3}. \end{align*} (:latexend:)

So to compute ($\overline{x}$), we find

(:latex:) \begin{align*} \overline{x} &= \frac{1}{M} \iint_R \rho(x,y)x\,dy\,dx
&= \frac{3}{128} \int_{x=0}^4 \int_{y=0}^{4x-x^2} (2x)x \, dy \, dx
&= \frac{3}{128} \int_{x=0}^4 \left(2x^2 y \bigg|_{y=0}^{4x-x^2} \right) \, dx
&= \frac{3}{128} \int_{x=0}^4 2x^2(4x-x^2) \, dx
&= \frac{3}{128} \left(2x^4 - \frac{2}{5}x^5 \right) \bigg|_{x=0}^4
&= \frac{3}{128} 512 - \frac{2048}{5}
&= \frac{3}{128} \cdot \frac{512}{5} = \frac{12}{5}. \end{align*} (:latexend:)

Similarly,

(:latex:) \begin{align*} \overline{y} &= \frac{1}{M} \iint_R \rho(x,y)y \, dy\,dx
&= \frac{3}{128} \int_{x=0}^4 \int_{y=0}^{4x-x^2} (2x)y \, dy \, dx
&= \frac{3}{128} \int_{x=0}^4 xy^2 \bigg|_{y=0}^{4x-x^2} \, dx
&= \frac{3}{128} \int_{x=0}^4 x(4x-x^2)^2 \, dx
&= \frac{3}{128} \int_{x=0}^4 (16x^3 - 8x^4 + x^5) \, dx
&= \frac{3}{128} \left(4x^4 - \frac{8}{5} x^5 + \frac{1}{6} x^6\right) \bigg|_{x=0}^4
&= \frac{3}{128} \cdot 1024 \left(1 - \frac{8}{5} + \frac{2}{3}\right) = \frac{8}{5}. \end{align*} (:latexend:)
...
(:latex:) \[ \overline{x} = 0. \] (:latexend:)

Taking the weighted average of the y-coordinates of the points gives

(:latex:) \begin{align*} \overline{y} &= \frac{\frac{1}{2}\pi R^2 \cdot \frac{4R}{3\pi} + 2RH \cdot \left(-\frac{H}{2}\right)}{\frac{1}{2}\pi R^2 + 2RH}
&= \frac{\frac{2}{3}R^3 - H^2R}{\frac{1}{2}\pi R^2 + 2RH}
&= \frac{4R^2 - 6H^2}{3 \pi R + 12H}. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \hbox{Surface area } &= \hbox{Perimeter } \cdot \hbox{ Centroid travel distance }
&= (2 \pi r) \cdot (2\pi R)
&= 4 \pi^2 r \cdot R. \end{align*} (:latexend:)

And the volume of the torus is

(:latex:) \begin{align*} \hbox{Volume } &= \hbox{Area } \cdot \hbox{ Centroid travel distance }
&= (\pi r^2) \cdot (2 \pi R)
&= 2 \pi^2 r^2 R. \end{align*} (:latexend:)
EXERCISES
