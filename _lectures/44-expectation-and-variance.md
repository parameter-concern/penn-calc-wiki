...
(:latex:) \begin{align*} \mathbb{E} &= \int_D x\rho(x)\,dx
&= \int_D x \, dP, \end{align*} (:latexend:)
...
(:latex:) \begin{equation*} \mathbb{E}[f(X)] = \int_D f(x)\rho(x)\,dx. \end{equation*} (:latexend:)
...
(:latex:) \begin{align*} \mathbb{E} &= \int_a^b x \cdot \frac{1}{b-a}\,dx
&= \frac{1}{b-a} \frac{x^2}{2} \bigg|^b_a
&= \frac{1}{b-a} \cdot \frac{1}{2}(b^2-a^2)
&= \frac{1}{b-a} \cdot \frac{1}{2}(b+a)(b-a)
&= \frac{1}{2}(a+b). \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \mathbb{E} &= \int_0^\infty t \alpha e^{-\alpha t}\, dt
&= \alpha \int_0^\infty te^{-\alpha t}\, dt. \end{align*} (:latexend:)

Using integration by parts, with

(:latex:) \begin{align*} u &= t & du &= dt
dv &= e^{-\alpha t} & v &= \frac{1}{-\alpha}e^{-\alpha t}, \end{align*} (:latexend:)

we find that

(:latex:) \begin{align*} \alpha \int_0^\infty te^{-\alpha t}\, dt &= \alpha \left(\frac{t}{-\alpha}e^{-\alpha t} - \int_0^{\infty} \frac{1}{-\alpha} e^{-\alpha t}\,dt \right)
&= \left(-te^{-\alpha t} - \frac{1}{\alpha} e^{-\alpha t}\right) \bigg|^\infty_0
&=(0-0)-(0 - \frac{1}{\alpha})
&= \frac{1}{\alpha}. \end{align*} (:latexend:)
Variance

Consider a random variable ($X$) with PDF ($\rho(x)$). The variance of ($X$), denoted ($\mathbb{V}$), is defined by

(:latex:) \begin{align*} \mathbb{V} &= \mathbb{E}\left[ \left( X-\mathbb{E}[X] \right)^2 \right]
&=\mathbb{E}[X^2] - \mathbb{E}[X]^2. \end{align*} (:latexend:)

In the notation of the lecture,

(:latex:) \begin{align*} \mathbb{V} &= \int_D (x-\mathbb{E})^2 \, dP
&= \int_D x^2 \, dP - \mathbb{E}^2. \end{align*} (:latexend:)
...
(:toggle hide show="Justification" box2b:)

Expanding out the expression and using the linearity of the integral, we find

(:latex:) \begin{align*} \int_D (x-\mathbb{E})^2 \, dP &= \int_D (x-\mathbb{E})^2 \rho(x) \, dx
...
(:latex:) \begin{align*} \mathbb{E}(X^2) &= \int_D x^2 \, dP
&= \int_{x=0}^\infty x^2 \alpha e^{-\alpha x} \, dx. \end{align*} (:latexend:)

Using integration by parts, with

(:latex:) \begin{align*} u &= x^2 & du &= 2x \, dx
dv &= \alpha e^{-\alpha x} \, dx & v &= -e^{-\alpha x}, \end{align*} (:latexend:)

we find

(:latex:) \begin{align*} \int_{x=0}^\infty x^2 \alpha e^{-\alpha x} \, dx &= -x^2 e^{-\alpha x} \bigg|_{x=0}^\infty + \int_{x=0}^\infty 2x e^{-\alpha x} \, dx. \end{align*} (:latexend:)
...
(:latex:) \[ \int_{x=0}^\infty x \alpha e^{-\alpha x} \, dx = \frac{1}{\alpha}, \] (:latexend:)

and so by dividing through by ($\alpha$), we have

(:latex:) \[ \int_{x=0}^\infty x e^{-\alpha x} \, dx = \frac{1}{\alpha ^2}. \] (:latexend:)

Putting this together, we have

(:latex:) \begin{align*} \int_{x=0}^\infty x^2 \alpha e^{-\alpha x} \, dx &= -x^2 e^{-\alpha x} \bigg|_{x=0}^\infty + \frac{2}{\alpha^2}
&= (0 - 0) + \frac{2}{\alpha^2}
&= \frac{2}{\alpha^2}. \end{align*} (:latexend:)

Finally, then, the variance is

(:latex:) \begin{align*} \mathbb{V} &= \int_D x^2 \, dP - \mathbb{E}^2
&= \frac{2}{\alpha^2} - \left(\frac{1}{\alpha}\right)^2
&= \frac{1}{\alpha^2}. \end{align*} (:latexend:)
Standard deviation

Consider a random variable ($X$) with PDF ($\rho(x)$). Then the standard deviation of ($X$), denoted ($\sigma_X$), is defined by

(:latex:) \begin{align*} \sigma_X &= \sqrt{V[X]}
&= \sqrt{E[X^2]-E[X]^2}
&= \sqrt{\int_D x^2\rho(x)\,dx - \left(\int_D x\rho(x) \,dx\right)^2}. \end{align*} (:latexend:)
Example

Find the standard deviation of ($X$), where ($X$) is uniformly distributed over ($\left[a,b\right]$). (:toggle hide show="Answer" box3:)

Again, recall that the PDF for the uniform distribution is ($\rho(x) = \frac{1}{b-a}$) for ($a \leq x \leq b$). Thus,

(:latex:) \begin{align*} E[X^2] &= \int_a^b x^2 \frac{1}{b-a} \,dx
&= \frac{1}{b-a} \frac{x^3}{3} \bigg|^b_a
&= \frac{1}{b-a} \cdot \frac{1}{3} (b^3-a^3)
&= \frac{1}{b-a} \cdot \frac{1}{3} (b-a)(b^2 +ba + a^2)
&= \frac{b^2+ab+a^2}{3} \end{align*} (:latexend:)

From the previous example, ($E[X] = \mu_X = \frac{a+b}{2}$). Thus,

(:latex:) \begin{align*} \sigma_X &= \sqrt{E[X^2] - E[X]^2}
&= \sqrt{ \frac{b^2+ba+a^2}{3} - \frac{b^2+2ba+a^2}{4}}
&= \sqrt{ \frac{b^2-2ab+a^2}{12}}
&= \frac{b-a}{\sqrt{12}}. \end{align*} (:latexend:)
Interpretations
...
(:latex:) \begin{equation*} \rho(x) = \frac{1}{\sigma \sqrt{2\pi}} e^{-\frac{1}{2} \left(\frac{x-\mu}{\sigma}\right)^2}. \end{equation*} (:latexend:)
...
The height of men in a certain population is normally distributed with mean ($\mu = 70$) inches and standard deviation ($\sigma = 2$) inches. If a man is chosen at random from the population, what is the probability that he is taller than 72 inches? (:toggle hide show="Answer" box4:)
...
(:latex:) \begin{align*} P(X \leq 72) &= P(X \leq 70)+P(70 \leq X \leq 72)
&= .5 + .34
&= .84. \end{align*} (:latexend:)

It follows that

(:latex:) \begin{align*} P(X > 72) &= 1 - P(X \leq 72)
&= 1 - .84
&= .16. \end{align*} (:latexend:)
