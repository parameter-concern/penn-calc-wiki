...
$$
 \arctan(x)= x - O(x^3) $$ as $$x \rightarrow 0$$.
...
<div id="box1e" class="answer-hidden"></div>
{: id="box1e" class="answer-hidden"}
...
requires that $$\vertx\vert<1$$. So we must do a little algebra to get the square root to be of this form. Factoring out a $$x^2$$ will do the trick:
...
<div id="box1f" class="answer-hidden"></div>
{: id="box1f" class="answer-hidden"}
...
$$
 O(f(x)) + O(g(x)) = O(f(x) + g(x)) \[ O(f(x))\cdot O(g(x)) = O(f(x)\cdot g(x))$$

Using these, show that $$
 O\left(\frac{5}{x}\right) + O\left(\frac{\ln(x^2)}{4x}\right) $$ simplifies to $$
 \displaystyle O\left(\frac{\ln x}{x}\right) $$

- Which of the following are in $$O(x^2)$$ as $$x \to 0 $$? 

$$
x \ln(1+x) \[ 5x^2+6x+1 \[1-e^{-x} \[ x \sqrt{x^2+4x^3+5x^6} \[x \sinh^2(3x) \[\frac{x^2}{\ln(1+x)} $$ 