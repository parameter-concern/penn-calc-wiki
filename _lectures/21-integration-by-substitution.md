...
(:latex:) \begin{align*} \int (u+v) \, dx &= \int u \, dx + \int v \, dx
\int (c u) \, dx &= c \int u \, dx \end{align*} (:latexend:)
...
(:latex:) \[ du = \frac{du}{dx} dx. \] (:latexend:)

Now if ($f = f(u)$) is a function of ($u$), then we find

(:latex:) \[ \int f(u) \, du = \int f(u(x)) \frac{du}{dx} \, dx \] (:latexend:)
...
(:latex:) \[ \int e^{\sin x} \cos x \, dx. \] (:latexend:)

(:toggle hide show="Answer" box1:)

Let ($u = \sin x$) and ($f(u) = e^u$). Then

(:latex:) \[ du = \frac{du}{dx} \, dx = \cos x \, dx, \] (:latexend:)

and

(:latex:) \begin{align*} \int e^{\sin x} \cos x \, dx &= \int f(\sin x) d(\sin x)
&= \int f(u) \, du
&= \int e^u \, du
&= e^u + C
&= e^{\sin x} + C. \end{align*} (:latexend:)
...
(:latex:) \[ \int x \sqrt{x-1} \, dx \] (:latexend:)
...
(:latex:) \begin{align*} \int x \sqrt{x-1} \, dx &= \int (u+1) \sqrt{u} \, du
&= \int (u+1)u^{1/2} \, du
&= \int u^{3/2} + u^{1/2} \, du
&= \frac{2}{5}u^{5/2} + \frac{2}{3}u^{3/2} + C
&= \frac{2}{5}(x-1)^{5/2} + \frac{2}{3}(x-1)^{3/2} + C. \end{align*} (:latexend:)
...
(:latex:) \[ \int \cot \theta \csc \theta \, d\theta. \] (:latexend:)
...
(:latex:) \begin{align*} \int \cot \theta \csc \theta \, d\theta &= \int \frac{\cos \theta}{\sin \theta} \cdot \frac{1}{\sin \theta} \, d\theta
&= \int \frac{\cos \theta}{\sin^2 \theta} \, d\theta. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \int \frac{\cos \theta}{\sin^2 \theta} \, d\theta &= \int \frac{1}{u^2} \, du
&= \int u^{-2} \, du
&= -u^{-1} + C
&= -\frac{1}{\sin \theta} + C
&= -\csc \theta + C. \end{align*} (:latexend:)

Of course, if one happened to remember the fact that

(:latex:) \[ \frac{d}{d\theta} \csc \theta = -\csc \theta \cot \theta, \] (:latexend:)
...
(:latex:) \[ \int \frac{dN}{N \ln(bN)} = \int -a \, dt. \] (:latexend:)
...
(:latex:) \[ du = \frac{1}{bN} \cdot b \, dN = \frac{dN}{N}. \] (:latexend:)

And so, the integral on the left above becomes

(:latex:) \begin{align*} \int \frac{dN}{N \ln(bN)} &= \int \frac{1}{u} \, du
&= \ln u + C
&= \ln \ln (bN) + C. \end{align*} (:latexend:)
...
(:latex:) \[ \ln \ln (bN) = -at + C \] (:latexend:)

Exponentiating twice and then dividing by ($b$) gives

(:latex:) \[ N = \frac{1}{b}e^{e^{-at+C}} = \frac{1}{b}e^{Ce^{-at}}. \] (:latexend:)
...
(:latex:) \[ \lim_{t \rightarrow \infty} N(t) = \frac{1}{b}. \] (:latexend:)
...
(:latex:) \[ \int 4 (2x+5)^4 dx. \] (:latexend:)
...
(:latex:) \[ \int \frac{(\ln x)^2}{x} \, dx. \] (:latexend:)

(:toggle hide show="Answer" box7:)

Here, a good inner function is ($u = \ln x$), because the derivative ($du = \frac{1}{x}\, dx$). Thus

(:latex:) \begin{align*} \int \frac{(\ln x)^2}{x} \, dx &= \int u^2 \, du
&= \frac{u^3}{3} + C
&= \frac{(\ln x)^3}{3} + C. \end{align*} (:latexend:)

Example

Compute

(:latex:) \[ \int \tan \theta \, d\theta. \] (:latexend:)

(:toggle hide show="Answer" box8:)

First, rewrite tangent in terms of sine and cosine:

(:latex:) \[ \int \tan \theta \, d \theta = \int \frac{\sin \theta}{\cos \theta} \, d \theta \] (:latexend:)
...
(:latex:) \begin{align*} \int \frac{\sin \theta}{\cos \theta} \, d \theta &= \int -\frac{1}{u} \, du
&= - \ln(u) + C
&= - \ln(\cos \theta) + C. \end{align*} (:latexend:)

Example

Compute

(:latex:) \[ \int x^5 \sqrt{1+x^3} \, dx. \] (:latexend:)

(:toggle hide show="Answer" box9:)

The logical choice of inner function is ($u = 1+x^3$), which gives ($du = 3x^2 \, dx$) and so

(:latex:) \[ dx = \frac{du}{3x^2} \] (:latexend:)

Substituting in, we find

(:latex:) \begin{align*} \int x^5 \sqrt{1+x^3} \, dx &= \int x^5 \sqrt{u} \frac{du}{3x^2}
&= \frac{1}{3} \int x^3 \sqrt{u} \, du. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \frac{1}{3} \int x^3 \sqrt{u} \, du &= \frac{1}{3} \int (u-1)\sqrt{u} \, du
&= \frac{1}{3} \int u^{3/2} - u^{1/2} \, du
&= \frac{1}{3} \left( \frac{2}{5}u^{5/2} - \frac{2}{3}u^{3/2} \right) + C
&= \frac{2}{15}(1+x^3)^{5/2} - \frac{2}{9}(1+x^3)^{3/2} + C. \end{align*} (:latexend:)
EXERCISES
