...
g(x) &= g(a) + g'(a)(x-a) + \frac{g(a)}{2!}(x-a)^2 + \dotsb. \end{align*} (:latexend:)

Since, by hypothesis, ($f(a) = g(a) = 0$), it follows that

(:latex:) \begin{align*} \lim_{x\rightarrow a} \frac{f(x)}{g(x)} &= \lim_{x \rightarrow a} \frac{ f(a) + f'(a)(x-a) + \frac{1}{2}f(a)(x-a)^2 + \dotsb}{g(a) + g'(a)(x-a) + \frac{1}{2}g(a)(x-a)^2+\dotsb}
...
&= \lim_{x \rightarrow a} \frac{f'(a) + (1/2)f(a)(x-a) + \dotsb }{g'(a) + (1/2) g(a)(x-a) + \dotsb}. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \lim_{x \rightarrow 0} & \frac{\sin x}{x}
\lim_{x \rightarrow 0} & \frac{1-\cos x}{x}. \end{align*} (:latexend:)

(:toggle hide show="Answer" box0:)

These are both in the ($\frac{0}{0}$) case, so differentiating numerator and denominator gives

(:latex:) \begin{align*} \lim_{x \rightarrow 0} \frac{\sin x}{x} &= \lim_{x \rightarrow 0} \frac{\cos x}{1}
&= 1. \end{align*} (:latexend:)

(:latex:) \begin{align*} \lim_{x \rightarrow 0} \frac{1-\cos x}{x} &= \lim_{x \rightarrow 0} \frac{\sin x}{1}
&= 0. \end{align*} (:latexend:)

Example Compute ($ \displaystyle \lim_{x \rightarrow 0} \frac{\tan x}{\arcsin x}$). (:toggle hide show="Answer" box0b:)

Since ($\tan 0 = \arcsin 0 = 0$), we are in the ($\frac{0}{0}$) case of L'Hopital's rule. Recall that

(:latex:) \begin{align*} \frac{d}{dx} \tan x &= \sec^2 x
\frac{d}{dx} \arcsin x &= \frac{1}{\sqrt{1-x^2}}. \end{align*} (:latexend:)

Thus, applying L'Hopital's rule gives

(:latex:) \begin{align*} \lim_{x \rightarrow 0} \frac{\tan x}{\arcsin x} &= \lim_{x \rightarrow 0} \frac{ \sec^2 x }{ \frac{1}{\sqrt{1-x^2}} }
&= \frac{1}{1}
&= 1. \end{align*} (:latexend:)
...
Example Compute ($\displaystyle \lim_{x \rightarrow 0} \frac{x^2 \ln( \cos x)}{\sin^2(3x^2)} $). (:toggle hide show="Answer" box0b1:)

We know all the relevant Taylor series for the functions in this problem, so that should be an easier method. We find

(:latex:) \begin{align*} \lim_{x\rightarrow 0} \frac{x^2 \ln(\cos x)}{\sin^2(3x^2)} &= \lim_{x \rightarrow 0} \frac{x^2 \ln \left(1-\frac{x^2}{2!} + \hbox{ HOT}\right)}{\left(3x^2 + \hbox{ HOT}\right)^2}
...
Example Compute ($\displaystyle \lim_{x \rightarrow 0} \frac{\ln x}{\frac{1}{x^2}}$). (:toggle hide show="Answer" box0c:)
...
(:latex:) \begin{align*} \lim_{x \rightarrow 0} \frac{\ln x}{x^{-2}} &= \lim_{x \rightarrow 0} \frac{\frac{1}{x}}{-2 x^{-3}}
&= \lim_{x\rightarrow 0} \frac{x^2}{-2}
&= 0. \end{align*} (:latexend:)

Example Use L'Hopital's rule to compute ($\displaystyle \lim_{x\rightarrow \pi} \frac{\sin(x)}{e^x \cos(x/2)}$). (:toggle hide show="Answer" box1:)
...
(:latex:) \begin{align*} \lim_{x\rightarrow \pi} \frac{\sin(x)}{e^x \cos(x/2)} &= \lim_{x\rightarrow \pi} \frac{[\sin(x)]'}{[e^x \cos(x/2)]'}
&= \lim_{x \rightarrow \pi} \frac{\cos(x)}{e^x \cos(x/2) -(1/2)e^x\sin(x/2)}
&= \frac{-1}{0 - (1/2)e^{\pi} \sin(\pi/2)}
&= 2e^{-\pi}. \end{align*} (:latexend:)
...
Example Compute ($\displaystyle \lim_{x \rightarrow 0} \frac{1}{\sin^2 x} - \frac{1}{x^2}$). (:toggle hide show="Answer" box1a:)

Getting a common denominator, and then Taylor expanding gives

(:latex:) \begin{align*} \lim_{x \rightarrow 0} \frac{1}{\sin^2x} - \frac{1}{x^2} &= \lim_{x\rightarrow 0} \frac{x^2 - \sin^2x}{x^2 \sin^2x }
...
(:latex:) \begin{align*} \lim_{x \rightarrow a} f(x)g(x) &= \lim_{x \rightarrow a} \frac{g(x)}{1/f(x)} \lim_{x \rightarrow a} f(x)g(x) &= \lim_{x \rightarrow a} \frac{f(x)}{1/g(x)} \end{align*} (:latexend:)
...
Example Compute ($\displaystyle \lim_{x \rightarrow 0^+} x \ln x$). (Here we use the one-sided limit ($x \rightarrow 0^+$) becuase ($\ln$) is only defined on the positive real numbers). (:toggle hide show="Answer" box1b:)
...
(:latex:) \begin{align*} \lim_{x \rightarrow 0^+} x \ln x &= \lim_{x \rightarrow 0^+} \frac{ \ln x}{x^{-1}}, \end{align*} (:latexend:)

which is of the form ($\frac{-\infty}{\infty}$), so applying L'Hopital's rule gives

(:latex:) \begin{align*} \lim_{x \rightarrow 0^+} \frac{ \ln x}{x^{-1}} &= \lim_{x \rightarrow 0^+} \frac{x^{-1}}{-x^{-2}}
...
(:latex:) \[ y = \lim_{x \rightarrow a} f(x)^{g\left(x\right)}, \] (:latexend:)
...
(:latex:) \begin{align*} \ln(y) &= \ln\left(\lim_{x\rightarrow a} f(x)^{g\left(x\right)}\right)
&= \lim_{x \rightarrow a} \ln \left(f(x)^{g\left(x\right)}\right)
&= \lim_{x \rightarrow a} g(x) \ln(f(x)) \end{align*} (:latexend:)
...
Example Compute ($\displaystyle \lim_{x \rightarrow \infty} x^{1/x}$). (:toggle hide show="Answer" box4:)

This is of the ($\infty^0$) form. Let ($\displaystyle y = \lim_{x \rightarrow \infty} x^{1/x}$). Then taking ($\ln$) gives

(:latex:) \begin{align*} \ln(y) &= \lim_{x \rightarrow \infty} \ln\left(x^{1/x}\right)
...
Example Compute ($\displaystyle \lim_{x \rightarrow 0^+} x^x$). (:toggle hide show="Answer" box1c:)

Letting ($\displaystyle y = \lim_{x \rightarrow 0^+} x^x$), and taking logarithms gives

(:latex:) \begin{align*} \ln y &= \lim_{x \rightarrow 0^+} x \ln x
&= 0, \end{align*} (:latexend:)
...
Example Compute ($\displaystyle \lim_{x \rightarrow \infty} \frac{\ln x}{\sqrt{x}}$). (:toggle hide show="Answer" box2:)

Both the numerator and denominator go to ($\infty$) as ($x \rightarrow \infty$), so applying L'Hopital's rule gives

(:latex:) \begin{align*} \lim_{x \rightarrow \infty} \frac{\ln x}{\sqrt{x}} &= \lim_{x \rightarrow \infty} \frac{x^{-1}}{\frac{1}{2}x^{-1/2}}
&= \lim_{x \rightarrow \infty} \frac{2}{\sqrt{x}}
&= 0. \end{align*} (:latexend:)

Example Compute ($\displaystyle \lim_{x \rightarrow \infty} \frac{e^{x}}{x^2}$). (:toggle hide show="Answer" box2a:)

The numerator and denominator both go to ($\infty$) as ($x \rightarrow \infty$), so this is the ($\frac{\infty}{\infty}$) case of L'Hopital's rule. It follows that (:latex:) \begin{align*} \lim_{x\rightarrow \infty} \frac{e^x}{x^2} &= \lim_{x\rightarrow \infty} \frac{[e^x]'}{[x^2]'}
&= \lim_{x\rightarrow \infty} \frac{e^x}{2x}
&= \lim_{x \rightarrow \infty} \frac{e^x}{2}
&= \infty. \end{align*} (:latexend:) Note that L'Hopital's rule was used twice here since ($\lim_{x\rightarrow \infty} \frac{e^x}{2x}$) is still the ($\frac{\infty}{\infty}$) case.
...
Example Compute ($\displaystyle \lim_{x \rightarrow \infty} \tanh x$). (:toggle hide show="Answer" box2b:)
...
(:latex:) \begin{align*} \lim_{x \rightarrow \infty} \tanh x &= \lim_{x \rightarrow \infty} \frac{\sinh x}{\cosh x}
&= \lim_{x \rightarrow \infty} \frac{\cosh x}{\sinh x}
&= \lim_{x \rightarrow \infty} \frac{\sinh x}{\cosh x}, \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \lim_{x \rightarrow \infty} \tanh x &= \lim_{x \rightarrow \infty} \frac{e^x - e^{-x}}{e^x + e^{-x}}
&= \lim_{x \rightarrow \infty} \frac{e^x(1 - e^{-2x}}{e^x(1 + e^{-2x})}
&= 1. \end{align*} (:latexend:)

Example Compute ($ \displaystyle \lim_{x \rightarrow \infty} \frac{x \ln x}{\ln(\cosh x)}$). (:toggle hide show="Answer" box2c:)

Using L'Hopital's rule, we find

(:latex:) \begin{align*} \lim_{x \rightarrow \infty} \frac{ x \ln x}{\ln (\cosh x) } &= \lim_{x \rightarrow \infty} \frac{\frac{x}{x} + 1 \cdot \ln x}{\frac{\sinh x}{\cosh x}}
&= \lim_{x \rightarrow \infty} \frac{1+ \ln x}{\tanh x}
&= \infty, \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \lim_{x \rightarrow \infty} f(x) &= \lim_{z \rightarrow 0} f(1/z). \end{align*} (:latexend:)
...
Example Compute ($\displaystyle \lim_{x \rightarrow \infty} \frac{\sin(1/x)}{1/x}$). (:toggle hide show="Answer" box3:)

Using the substitution ($z = 1/x$), the limit becomes

(:latex:) \begin{align*} \lim_{x \rightarrow \infty} \frac{\sin(1/x)}{1/x} &= \lim_{z \rightarrow 0} \frac{\sin(z)}{z}
&= 1, \end{align*} (:latexend:) as was shown in the last module.
EXERCISES

Compute the following limits. Should you use l'Hopital's rule or Taylor expansion?

(:latex:) \[\displaystyle \lim_{x \to 2} \frac{x^3+2x^2-4x-8}{x-2} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to \pi/3} \frac{1-2\cos x}{\pi -3x} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to \pi} \frac{4 \sin x \cos x}{\pi - x} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to 9} \frac{2x-18}{\sqrt{x}-3} \] (:latexend:)

(:latex:) \[\lim_{x \to 0} \frac{e^x - \sin x -1}{x^2-x^3} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to 1} \frac{\cos (\pi x/2)}{1 - \sqrt{x}} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to 4} \frac{3 - \sqrt{5+x}}{1 - \sqrt{5-x}} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x\rightarrow 0} \left(\frac{1}{x}-\frac{1}{\ln (x+1)}\right) \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to \pi/2} \frac{\sin x \cos x}{e^x\cos 3x} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \rightarrow +\infty} \frac {\ln x}{e^x} \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to +\infty} x \ln\left(1+ \frac{3}{x}\right) \] (:latexend:)

(:latex:) \[\displaystyle \lim_{x \to +\infty} \frac{(\ln x)(\sinh x)}{(x-1)e^x} \] (:latexend:) 