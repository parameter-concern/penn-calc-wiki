...
$$
V = \left(\frac{2L}{3}\right)^2 \frac{L}{6} = \frac{2L^3}{27}.
$$

**Example**

Classify the critical points of $$f(x) = x^3 - 6x^2+9x-5$$. <button class="toggle" data-box="#box2">Answer</button>
...
Suppose a firm producing widgets expects to sell $$3000-10p^2$$ units (where $$p$$ is the price of the unit). What price $$p$$ should the firm set to maximize revenue (note that revenue here is just price times quantity sold)? <button class="toggle" data-box="#box3">Answer</button>
...
Find the global extrema of $$f(x) = x^3 - 6x^2+9x-5$$ on the interval $$\left[2,4\right]$$. <button class="toggle" data-box="#box4">Answer</button>
...
$$
m = \frac{\displaystyle \sum_i x_i y_i}{\displaystyle \sum_i x_i^2}.
$$

Applying the second derivative test, we compute

$$
\frac{d^2S}{dm^2} = 2 \sum_i x^2_i > 0,
$$
...
u = u_\mathrm{max} \left( 1 - \frac{v}{v_\mathrm{max}} \right)
$$
...
p = p_\mathrm{max} \left( 1 - \frac{n}{n_\mathrm{max}} \right)
$$
