...
Limit $$\displaystyle \lim_{x \rightarrow a} f(x) = L$$ if and only if for every $$\epsilon>0$$ there exists $$\delta>0$$ such that $$\vertf(x)-L\vert<\epsilon$$ whenever $$0<\vertx-a\vert<\delta$$. If there is no such $$L$$, then the limit does not exist.
...
We must show that for any given $$\epsilon>0$$, there exists $$\delta$$ (which depends on $$\epsilon$$$ such that $$0<\vertx-3\vert<\delta$$ implies $$\vertx^2-9\vert<\epsilon$$.

Let $$\epsilon>0$$ be given. A little bit of algebra shows that

$$
 \vertx^2-9\vert = \vertx-3\vert \cdot \vertx+3\vert. $$

We get to control $$\vertx-3\vert$$ with $$\delta$$. We also have (by using the triangle inequality) that

$$
 \vertx+3\vert = \vertx-3+6\vert \leq \vertx-3\vert+6 < \delta + 6. $$

Thus,

$$
 \vertx^2-9\vert = \vertx-3\vert\cdot \vertx+3\vert < \delta \cdot (\delta + 6). $$
...
$$
 \vertx^2-9\vert < \delta \cdot (\delta + 6) \leq \frac{\epsilon}{7} \cdot 7 = \epsilon, $$
