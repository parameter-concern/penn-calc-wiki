...
(:latex:) \begin{equation*} du = \frac{du}{dx}dx. \end{equation*} (:latexend:)
...
(:latex:) \begin{align*} 2x(dx) + 2y(dy) &= 0
2y(dy) &= -2x(dx)
\frac{dy}{dx} &= \frac{-2x}{2y}
&= -\frac{x}{y}. \end{align*} (:latexend:)
...
(:latex:) \[ \hbox{MRS} = -\frac{dY}{dX}. \] (:latexend:)

Find MRS for the utility function given above, ($U(X,Y) = Y^2(X-3)$). Then calculate the MRS at the points ($(4,2)$) and ($(7,1)$) and interpret the results. (:toggle hide show="Answer" box1b:)

Using implicit differentiation, we find

(:latex:) \begin{align*} Y^2(X-3) &= C
2Y\, dY \,(X-3) + Y^2 \, dX &= 0. \end{align*} (:latexend:)

Now solving for ($-\frac{dY}{dX}$) we find

(:latex:) \[ \hbox{MRS} = -\frac{dY}{dX} = \frac{Y^2}{2Y(X-3)} = \frac{Y}{2(X-3)}. \] (:latexend:)
...
(:latex:) \begin{equation*} ye^x+x\ln(y) = e. \end{equation*} (:latexend:)

(:toggle hide show="Answer" box2:)

Taking the differential (remembering the product rule) gives

(:latex:) \begin{equation*} (dy)e^x+y(e^xdx)+(dx)\ln(y) + x\left(\frac{1}{y}dy\right)= 0. \end{equation*} (:latexend:)
...
(:latex:) \begin{equation*} \left(e^x+\frac{x}{y}\right)dy = \left(-ye^x-\ln(y)\right)dx. \end{equation*} (:latexend:)

Finally, dividing through gives

(:latex:) \begin{equation*} \frac{dy}{dx} = \frac{-ye^x-\ln(y)}{e^x+x/y}. \end{equation*} (:latexend:)
Related rates
...
Suppose a 10 foot ladder is leaning against a wall. The base of the ladder starts sliding away from the wall at a rate of 4 feet per second. At the moment when the base of the ladder is 6 feet away from the wall, at what rate is the top of the ladder sliding down the wall? (:toggle hide show="Answer" box3:)
...
(:latex:) \begin{equation*} 2xdx+2ydy = 0, \end{equation*} (:latexend:)
...
(:latex:) \begin{equation*} \frac{dy}{dt} = \frac{dy}{dx}\cdot \frac{dx}{dt}. \end{equation*} (:latexend:)

Since ($\frac{dy}{dx} = -\frac{3}{4}$) and ($\frac{dx}{dt} = 4$) (given in the problem), it follows that

(:latex:) \begin{equation*} \frac{dy}{dt} = \frac{dy}{dx}\cdot \frac{dx}{dt} = \left(-\frac{3}{4}\right) \cdot 4 = -3. \end{equation*} (:latexend:)
...
(:latex:) \begin{align*} r^2 v &= C
2r \, dr \, v + r^2 \, dv &= 0
dr &= -\frac{r^2}{2rv} \, dv = -\frac{r}{2v} \, dv. \end{align*} (:latexend:)

So (:latex:) \[ \frac{dr}{dv} = -\frac{r}{2v}. \] (:latexend:)

Now, using the chain rule and a few facts from physics gives that

(:latex:) \begin{align*} \frac{dr}{dt} &= \frac{dr}{dv} \frac{dv}{dt}
&= -\frac{r}{2v} \frac{dv}{dt}
&= -\frac{rg}{2v}
&= -\frac{rg}{2(v_0 + gt)}. \end{align*} (:latexend:)
...
(:latex:) \[ \frac{du}{u} = d(\ln u). \] (:latexend:)
...
where ($V$) is voltage across the resistor, ($I$) is the current, and ($R$) is the resistance of the resistor. If the voltage across a variable resistor is fixed, find the relationship between the relative rates change of resistance and current. (:toggle hide show="Answer" box4:)

Beginning with Ohm's law and taking the differential gives

(:latex:) \begin{align*} V &= IR
dV &= R \, dI + I \, dR. \end{align*} (:latexend:)

Now, note that ($dV = 0$) since voltage was assumed to be constant. Then dividing through by ($IR$) gives

(:latex:) \begin{align*} R \, dI + I \, dR &= 0
\frac{R \, dI}{IR} + \frac{I \, dR}{IR} &= 0
\frac{dI}{I} + \frac{dR}{R} &= 0
\frac{dI}{I} = -\frac{dR}{R}. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} V &= \frac{4}{3}\pi r^3 & dV &= 4\pi r^2 \, dr
A &= 4\pi r^2 & dA &= 8\pi r \, dr. \end{align*} (:latexend:)

Then the relative rates of change of volume and area are

(:latex:) \begin{align*} \frac{dV}{V} &= \frac{4\pi r^2 \, dr}{\frac{4}{3}\pi r^3}
&= 3 \frac{dr}{r}. \end{align*} (:latexend:)

and

(:latex:) \begin{align*} \frac{dA}{A} &= \frac{8 \pi r \, dr}{4 \pi r^2}
&= 2 \frac{dr}{r}. \end{align*} (:latexend:)

Thus, the relative rate of change of volume is ($\frac{3}{2}$) that of area. Written another way,

(:latex:) \[ \frac{dV/V}{dA/A} = \frac{3}{2}. \] (:latexend:)

For a cube of side length ($s$), it turns out the same holds. The volume and area are

(:latex:) \begin{align*} V &= s^3 & dV &= 3s^2 \, ds
A &= 6 s^2 & dA &= 12 s \, ds. \end{align*} (:latexend:)
...
(:latex:) \[ E = -\frac{dQ/Q}{dP/P}. \] (:latexend:)
