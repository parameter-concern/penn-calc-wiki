...
(:latex:) \[ \int f(x) \, dx = \int f(x(\theta)) \frac{dx}{d\theta} \, d\theta, \] (:latexend:)
...
(:latex:) \[ \int \frac{dx}{1+x^2}. \] (:latexend:) (:toggle hide show="Answer" box1:)
...
(:latex:) \begin{align*} \int \frac{dx}{1+x^2} &= \int \frac{\sec^2\theta\, d\theta}{1+\tan^2\theta}
&= \int \frac{\sec^2\theta\, d\theta}{\sec^2\theta}
&= \int d\theta
&= \theta + C
&= \arctan(x) + C. \end{align*} (:latexend:)

The last line comes from our original substitution:

(:latex:) \[ x = \tan \theta \quad \Leftrightarrow \quad \arctan x = \theta. \] (:latexend:)
...
(:latex:) \[ \int \sqrt{1-x^2} \, dx. \] (:latexend:) (:toggle hide show="Answer" box2:)

According to the above guide, the substitution to make is ($x = \sin \theta$). Then ($dx = \cos \theta \, d\theta$), and it follows that

(:latex:) \begin{align*} \int \sqrt{1-x^2} \, dx &= \int \sqrt{1-\sin^2\theta} \cos \theta \, d\theta
&= \int \sqrt{\cos^2 \theta} \cos \theta \, d\theta
&= \int \cos^2 \theta \, d\theta. \end{align*} (:latexend:)

Now using the power reduction identity for cosine, we have

(:latex:) \begin{align*} \int \cos^2 \theta \, d\theta &= \int \frac{1}{2}(1+\cos(2 \theta)) \, d\theta
&= \frac{\theta}{2} + \frac{1}{4}\sin(2 \theta) + C. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \sin 2 \theta &= 2 \sin \theta \cos \theta
&= 2 x \sqrt{1-x^2} \end{align*} (:latexend:)
...
(:latex:) \[ \int \sqrt{1-x^2} = \frac{1}{2}\arcsin x + \frac{1}{2} x \sqrt{1-x^2} + C. \] (:latexend:)

Example

Compute

(:latex:) \[ \int \frac{dx}{\sqrt{1-x^2}}. \] (:latexend:) (:toggle hide show="Answer" box2a:)
...
(:latex:) \begin{align*} \int \frac{dx}{\sqrt{1-x^2}} &= \int \frac{\cos \theta\, d\theta}{\sqrt{1-\sin^2\theta}}
&= \int \frac{\cos \theta\, d\theta}{\sqrt{\cos^2\theta}}
&= \int d \theta
&= \theta + C
&= \arcsin(x)+C. \end{align*} (:latexend:)
...
(:latex:) \[ \int \frac{dx}{x^2\sqrt{x^2+4}} \] (:latexend:) (:toggle hide show="Answer" box3a:)
...
(:latex:) \[ 4 \tan^2 \theta + 4 = 4(\tan^2 \theta + 1). \] (:latexend:)
...
(:latex:) \begin{align*} \int \frac{dx}{x^2 \sqrt{x^2+4}} &= \int \frac{2 \sec^2 \theta \, d \theta}{4 \tan^2 \theta \sqrt{4 \tan^2 \theta + 4}}
&=\int \frac{2 \sec^2 \theta \, d \theta}{4 \tan^2 \theta \sqrt{4(\tan^2 \theta + 1)}}
&= \frac{1}{4} \int \frac{\sec^2 \theta \, d \theta}{\tan^2 \theta \sec \theta}. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \frac{1}{4} \int \frac{\sec \theta \, d \theta}{\tan^2 \theta} &= \frac{1}{4} \int \frac{1}{\cos \theta} \cdot \frac{\cos^2 \theta}{\sin^2 \theta} \, d\theta
&= \frac{1}{4} \int \frac{\cos \theta \, d \theta}{\sin^2 \theta}. \end{align*} (:latexend:)

This we can handle with a substitution of ($u = \sin \theta$) and ($du = \cos \theta \, d \theta$), which gives

(:latex:) \begin{align*} \frac{1}{4} \int \frac{\cos \theta \, d \theta}{\sin^2 \theta} &= \frac{1}{4} \int \frac{du}{u^2}
&= \frac{1}{4}\left(-\frac{1}{u}\right) + C
&= -\frac{1}{4u} + C
&= -\frac{1}{4 \sin \theta} + C \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \int \frac{dx}{x^2 \sqrt{x^2+4}} &= -\frac{1}{4 \sin \theta} + C
&= -\frac{\sqrt{x^2+4}}{4x} + C. \end{align*} (:latexend:)

Example

Compute

(:latex:) \[ \int \frac{dx}{4+9x^2}. \] (:latexend:) (:toggle hide show="Answer" box3b:)
...
(:latex:) \begin{align*} \int \frac{dx}{4+9x^2} &= \frac{2}{3} \int \frac{\sec^2\theta\, d\theta}{4+9(4/9)\tan^2\theta}
...
&= \frac{1}{6} \theta + C
&= \frac{1}{6} \arctan(\frac{3}{2}x) + C. \end{align*} (:latexend:)
...
(:latex:) \[ \int \frac{dx}{\sqrt{3+2x-x^2}} \] (:latexend:) (:toggle hide show="Answer" box3c:)

Start by completing the square for the quadratic:

(:latex:) \begin{align*} 3+2x-x^2 &= -x^2+2x+3
&= -(x^2-2x) + 3
&= -(x^2-2x+1) + 4
&= -(x-1)^2 + 4
&= 4-(x-1)^2. \end{align*} (:latexend:)

So we can rewrite the integral as

(:latex:) \begin{align*} \int \frac{dx}{\sqrt{3+2x-x^2}} &= \int \frac{dx}{\sqrt{4-(x-1)^2}}

&= \int \frac{du}{\sqrt{4-u^2}}, \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \int \frac{du}{\sqrt{4-u^2}} &= \int \frac{2 \cos \theta \, d \theta}{\sqrt{4-4\sin^2 \theta}}
&= \int \frac{2 \cos \theta \, d \theta}{\sqrt{4}\sqrt{1-\sin^2 \theta}}
&= \int \frac{2 \cos \theta \, d \theta}{2 \cos \theta}
&= \int d \theta
&= \theta + C. \end{align*} (:latexend:)

Solving our original substitution for ($\theta$), we see that

(:latex:) \begin{align*} \theta &= \arcsin\left(\frac{u}{2}\right)
&= \arcsin\left(\frac{x-1}{2}\right). \end{align*} (:latexend:)

So the final answer is

(:latex:) \[ \int \frac{dx}{\sqrt{3+2x-x^2}} = \arcsin \left(\frac{x-1}{2}\right) + C. \] (:latexend:)
...
(:latex:) \begin{align*} \sinh(\theta) &= \frac{e^\theta - e^{-\theta}}{2}
\cosh(\theta) &= \frac{e^\theta+e^{-\theta}}{2}. \end{align*} (:latexend:)
...
(:latex:) \[ \int \frac{dx}{\sqrt{1+x^2}}. \] (:latexend:) (:toggle hide show="Answer" box4:)
...
(:latex:) \begin{align*} \int \frac{dx}{\sqrt{1+x^2}} &= \int \frac{ \sec^2 \theta \, d \theta}{\sqrt{1+\tan^2 \theta}}
&= \int \sec \theta \, d\theta. \end{align*} (:latexend:)
...
(:latex:) \begin{align*} \int \frac{dx}{\sqrt{1+x^2}} &= \int \frac{\cosh u \, du}{\sqrt{1+\sinh^2 u}}
&= \int \frac{\cosh u \, du}{\cosh u}
&= \int du
&= u + C
&= \arcsinh x + C. \end{align*} (:latexend:)
...
(:latex:) \[ \int \sqrt{1+x^2}\,dx. \] (:latexend:) (:toggle hide show="Answer" box4a:)

Using the hyperbolic trigonometric substitution ($x = \sinh(\theta)$) gives

(:latex:) \begin{align*} \int \sqrt{1+x^2}\,dx &= \int \sqrt{1+\sinh^2\theta} \cosh \theta\, d\theta
...
&= \frac{1}{2} \sinh^{-1} x + \frac{1}{2} x \sqrt{1+x^2} + C. \end{align*} (:latexend:)
Blow-ups
...
(:latex:) \[ \frac{dP}{dt} = b^2 + a^2P^2, \] (:latexend:)

for constants ($a$) and ($b$) (we square them to ensure that they are positive). Solve this differential equation and find where it has a blow-up. (:toggle hide show="Answer" box5:)

This is a separable equation. Separating the variables and integrating both sides gives

(:latex:) \[ \int \frac{dP}{b^2 + a^2P^2} = \int dt. \] (:latexend:)

On the left, we can use the trigonometric substitution

(:latex:) \begin{align*} P &= \frac{b}{a} \tan \theta
dP &= \frac{b}{a} \sec^2 \theta \, d \theta. \end{align*} (:latexend:)

Note then that ($\theta = \arctan \frac{a}{b}P$). This gives

(:latex:) \begin{align*} \int \frac{dP}{b^2+a^2P^2} &= \int \frac{\frac{b}{a}\sec^2 \theta \, d \theta}{b^2(1+\tan^2 \theta)}
&= \frac{b}{a} \int \frac{\sec^2 \theta \, d \theta}{b^2 \sec^2 \theta}
&= \frac{1}{ab} \int d \theta
&= \frac{1}{ab} \theta
&= \frac{1}{ab} \arctan \frac{a}{b}P. \end{align*} (:latexend:)

(leaving off the constant for now). On the right side we get ($t + C$), so

(:latex:) \[ \frac{1}{ab} \arctan \frac{a}{b}P = t + C. \] (:latexend:)

Solving this for ($P$) gives

(:latex:) \[ P(t) = \frac{b}{a} \tan(abt + C). \] (:latexend:)

If initial profits, at ($t=0$), are 0, then ($C=0$), so the final answer is

(:latex:) \[ P(t) = \frac{b}{a} \tan(abt). \] (:latexend:)
